...
public class OptimalSolutionProvider {
	...
	public void init(EarliestArrivalProblem problem, double agentMaxSpeed,
	...
	public int getOptimalSolutionCost() {
		if (!isInitialized) {
			throw new RuntimeException(
					"OptimalSolutionProvider used without inititalization");
		}
	...
	private int calculateFeasibleOptimisticSolution() {
		...
		for (int i = 0; i < problem.nAgents(); i++) {
			...
			trajectories[i] = calculateTrajectory(graphPath,
					(int) agentMaxSpeed);
		}
	...
	private EvaluatedTrajectory calculateTrajectory(
		...
		EvaluatedTrajectory traj = new RVOTrajectory(trajectory, 1,
				shortestPath.getEndVertex(), trajectory.size());
		return traj;
	}
	...
	private void visualizeOptimalSolution() {
		VisManager.registerLayer(TrajectoriesLayer.create(
				...
				}, new tt.euclid2i.vis.ProjectionTo2d(), 1, 10000, 6, 's'));
	}
}
